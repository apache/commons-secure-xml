 ...
 *
 * <p>
 * Both views are produced from the same secure reader, so a caller reaching the parser through either the SAX 2 ({@link #getXMLReader()}) or the legacy
 ...
 *
 * <p>
 * The secure reader is computed lazily on first access and cached: securing an {@link XMLReader} can install a new wrapper (Android's Expat path), so
 * every parse must run through the same instance. The {@code parse(...)} overloads inherited from {@link SAXParser} dispatch virtually to
 * {@link #getXMLReader()}
 * and {@link #getParser()}, so they too run through the secure views without further overrides.
 * </p>
 */
