 ...
 *
 * <p>
 * Each {@code secure*} test asserts the library blocks the payload;
 ...
 * A payload therefore has to exceed the largest of these.
 * </p>
 *
 * <p>
 * Every payload shares one six-level x10 {@link #DTD} (declaring entities is free until they are referenced) and varies only the body it expands:
 * </p>
 *
 * <ul>
 ...
 *
 * <p>
 * The XSLT payload spreads those same {@code 120,000} expansions over two literal result elements with content {@link #CONTENT_60K} rather than one text node,
 * because XSLTC caps a compiled literal at 65,535 bytes; see {@link #xsltPayload()}.
 * A parser counts expansions across the whole document, so the split changes nothing on the secure side.
 * </p>
 *
 * <p>
 * Why a single character {@code "A"}: it makes the expanded size equal the expansion count, so a payload's size maps directly onto each parser's limit, and
 * (being ASCII) onto XSLTC's byte-counted constant-pool ceiling as well.
 * </p>
 */
class BillionLaughsTest {
    ...
    /**
     * Shared DTD for every payload: a six-level x10 ladder, {@code &lol1;} through {@code &lol6;} ({@code &lol6;} expands to 1,000,000). Declaring an entity
     * costs nothing until it is referenced, so the DTD is identical on every platform and only the expanded body ({@link #content()}) varies.
     */
     ...
     *
     * <p>
     * The controls prove the secure test blocked a payload that would otherwise parse, so they must use the very payload the secure test blocks.
     ...
     * cannot be parsed even without securing, leaving nothing to prove.
     * </p>
     */
 ...
 */
     ...
     *
     * <p>
     * XSLTC compiles each literal text node into a class-file string constant, and a {@code CONSTANT_Utf8} entry holds at most 65,535 bytes.
     * We split the payload into two constants to still trigger the Xerces 100k expansion limit, but without any text node above 64 KiB.
     * </p>
     *
     * <p>
     * Android keeps the single {@link #CONTENT_9M} body: libexpat aborts during the parse, so no translet is ever compiled.
     * </p>
     */
